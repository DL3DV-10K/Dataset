...
def download(download_list: list, output_dir: str, is_clean_cache: bool):
    ...
    for item in tqdm(download_list, desc='Downloading'):
        ...
        if os.path.exists(zip_path) or os.path.exists(output_path):
            succ_count += 1
            continue
        ...
        if succ:
            ...
            try:
                ...
                print(f"ERROR: {zip_path} is not a zip file. Download failed.")
                continue
            except Exception as e:
...
def download_dataset(args):
    ...
    return download(download_list[:500], output_dir, is_clean_cache)
